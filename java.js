$(function()
{
  ...
  // need to update ticket list when next page / prev page buttons are clicked 
  // they do not load a new html page for some stupid reason... :(
  $('.prev_page, .next_page').click(function(){
    setInterval(function(){
      tCountHigh = $('div.offset.ticketlist-total-count > b:nth-child(2)');
      tCountLow = $('div.offset.ticketlist-total-count > b:first');
      ticketCount = parseInt($(tCountHigh).text()) 
                      - parseInt($(tCountLow).text()) + 1;
      $.ajax({
        url: "https://msoe.freshservice.com/helpdesk/tickets.json",
        success: function(result){
